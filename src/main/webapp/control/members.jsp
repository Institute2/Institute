...
<body>
						...
						<c:forEach items="${list}" var="Member">
							<tr class="gradeA">
								<td><input name="name" value="${Member.name}"
									...
									value="${Member.id}" class="form-control" type="hidden">
									<input name="pictureId" value="${Member.picture.id}"
									class="form-control" type="hidden"></td>
								<td><input name="zhiwu" value="${Member.zhiwu}"
									class="form-control"></td>
								<td><input name="xibie" value="${Member.xibie}"
									class="form-control"></td>
								<td><textarea name="jianjie" class="form-control" rows="2">${Member.jianjie}</textarea></td>
								<%-- <td><a
									href="${pageContext.request.contextPath}/control/picture/getPicture.do?id=${Member.picture.id}">
										<button type="button" class="btn btn-outline btn-warning">修改图片</button>
								</a></td> --%>
								<td><input type="hidden" name="updatePicture"
									value="<%=basePath%>control/picture/getPicture.do?id=${Member.picture.id}">
									<button type="button" class="btn btn-outline btn-warning updatePicture">
										<img style="width:30px;height:30px"
											src="upload/${Member.picture.link}">
									</button></td>
								<td>
									<button type="button"
					...
					<table class="table table-striped table-bordered table-hover">
						...
						<tbody>
							<tr class="gradeA">
								...
								<td>
									<button class="btn btn-outline btn-success add">添加</button>
								</td>
							</tr>
						</tbody>
					</table>
	...
	<script src="control/js/member.js"></script>
